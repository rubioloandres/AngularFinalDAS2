  ...
  export class PricetableComponent implements OnInit, OnDestroy {
    ...
    agregarDatosSucursal(suc: Sucursal): SucursalTablaPrecio {
      const sucursalTablaPrecio: SucursalTablaPrecio = {
        ...
        menorPrecioTotal : suc.menorPrecioTotal,
    ...
    loadColumns() {
      ...
      this.listaSucursalesOrdenadas = this.listaSucursales.filter(suc => suc.menorPrecioTotal !== true).sort(  (suc1, suc2) => {
        if ( suc1.cantidadDeProductosConPrecioMasBajo > suc2.cantidadDeProductosConPrecioMasBajo) {  return -1; }
        if ( suc1.cantidadDeProductosConPrecioMasBajo < suc2.cantidadDeProductosConPrecioMasBajo) {  return 1; }
        return 0;
      });

      const mejoresPrecios = this.listaSucursales.filter(suc => suc.menorPrecioTotal === true);
    ...
    cambiarSucursalSeleccionada(suc: SucursalTablaPrecio, indexSuc: number, event: any) {

      const found = this.displayedColumns.find(s => s === ('sucursal' + (indexSuc + 1)));

      if ( found !== undefined ) {
         alert('La sucursal ya se muestra en la tabla');
